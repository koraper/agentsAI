...
description: 빌드 자동화된 billing 시스템 위한 recurring payments, invoicing, 구독 lifecycle, 및 dunning 관리. Use 때 implementing 구독 billing, automating invoicing, 또는 managing recurring payment 시스템.
---

# Billing 자동화

마스터 자동화된 billing 시스템 포함하여 recurring billing, invoice 세대, dunning 관리, proration, 및 tax 계산.

## 때 에 Use This Skill

- Implementing SaaS 구독 billing
- Automating invoice 세대 및 전달
- Managing 실패 payment 복구 (dunning)
- Calculating prorated charges 위한 plan 변경합니다
- 처리 sales tax, VAT, 및 GST
- 처리 usage-based billing
- Managing billing 순환합니다 및 renewals

## 핵심 개념

### 1. Billing 순환합니다
**일반적인 Intervals:**
- Monthly (most 일반적인 위한 SaaS)
- Annual (discounted long-term)
- Quarterly
- Weekly
- 사용자 정의 (usage-based, per-seat)
...
### 3. Dunning 관리
자동화된 프로세스 에 recover 실패 payments 통해:
- 재시도 예약합니다
- 고객 알림
- Grace periods
- 계정 restrictions
...
- **참조/billing-순환합니다.md**: Billing 사이클 관리
- **참조/dunning-관리.md**: 실패 payment 복구
- **참조/proration.md**: Prorated charge calculations
- **참조/tax-계산.md**: Tax/VAT/GST 처리
- **참조/invoice-lifecycle.md**: Invoice 상태 관리
- **자산/billing-상태-machine.yaml**: Billing 워크플로우
- **자산/invoice-템플릿.html**: Invoice 템플릿
- **자산/dunning-정책.yaml**: Dunning 구성

## 최선의 관행

1. **Automate Everything**: Minimize manual intervention
2. **명확한 Communication**: Notify customers of billing 이벤트
3. **유연한 재시도 Logic**: Balance 복구 와 함께 고객 experience
4. **Accurate Proration**: Fair 계산 위한 plan 변경합니다
5. **Tax Compliance**: Calculate 올바른 tax 위한 jurisdiction
6. **Audit Trail**: Log 모든 billing 이벤트
7. **Graceful Degradation**: Handle 엣지 cases 없이 breaking

## 일반적인 Pitfalls

- **올바르지 않은 Proration**: Not accounting 위한 부분 periods
- **Missing Tax**: Forgetting 에 add tax 에 invoices
- **Aggressive Dunning**: Canceling 또한 빠르게
- **아니요 알림**: Not informing customers of 실패
- **Hardcoded 순환합니다**: Not supporting 사용자 정의 billing dates
