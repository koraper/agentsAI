---
description: Static Application 보안 테스트 (SAST) 위한 code vulnerability analysis 전반에 걸쳐 multiple languages 및 frameworks
...
Static Application 보안 테스트 (SAST) 위한 포괄적인 code vulnerability detection 전반에 걸쳐 multiple languages, frameworks, 및 보안 patterns.

## Capabilities

- **Multi-language SAST**: Python, JavaScript/TypeScript, Java, Ruby, PHP, Go, Rust
- **Tool 통합**: Bandit, Semgrep, ESLint 보안, SonarQube, CodeQL, PMD, SpotBugs, Brakeman, gosec, cargo-clippy
...
- **Custom rule authoring**: Semgrep 패턴 development 위한 organization-specific 보안 policies

## When 에 Use This Tool

Use 위한 코드 리뷰 보안 analysis, injection vulnerabilities, hardcoded secrets, framework-specific patterns, custom 보안 policy enforcement, pre-배포 validation, legacy code assessment, 및 규정 준수 (OWASP, PCI-DSS, SOC2).

**Specialized tools**: Use `security-secrets.md` 위한 고급 credential scanning, `security-owasp.md` 위한 Top 10 mapping, `security-api.md` 위한 REST/GraphQL endpoints.
...
### JavaScript/TypeScript: ESLint 보안
...
**VULNERABLE**: String formatting/concatenation 와 함께 user input 에서 SQL queries
...
**VULNERABLE**: Direct HTML manipulation 와 함께 unsanitized user input (innerHTML, outerHTML, document.write)
...
**VULNERABLE**: Hardcoded API keys, passwords, tokens 에서 source code
...
**VULNERABLE**: Opening files 사용하여 unsanitized user input
...
**VULNERABLE**: pickle.loads(), yaml.load() 와 함께 untrusted data
...
**VULNERABLE**: os.system() 또는 subprocess 와 함께 shell=True 및 user input
...
**VULNERABLE**: random module 위한 보안-중요한 operations
...
**VULNERABLE**: @csrf_exempt, DEBUG=True, weak SECRET_KEY, missing 보안 middleware
...
**VULNERABLE**: Missing helmet, CORS wildcard, no 속도 제한
...
1. **Run early 및 often** - Pre-commit hooks 및 CI/CD
2. **Combine multiple tools** - Different tools catch different vulnerabilities
3. **Tune false positives** - Configure exclusions 및 thresholds
4. **Prioritize findings** - Focus 에 중요한/HIGH first
5. **Framework-aware scanning** - Use specific rulesets
6. **Custom rules** - Organization-specific patterns
7. **개발자 training** - 안전한 coding practices
...
- **보안-secrets.md** - 고급 credential detection
- **보안-owasp.md** - OWASP Top 10 assessment
- **보안-api.md** - API 보안 테스트
- **보안-scan.md** - 포괄적인 보안 scanning
