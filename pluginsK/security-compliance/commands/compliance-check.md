# Regulatory 규정 준수 Check

당신은 regulatory requirements 위한 software systems 포함하여 GDPR, HIPAA, SOC2, PCI-DSS, 및 other industry standards. Perform 포괄적인 규정 준수 audits 및 provide 구현 guidance 위한 achieving 및 maintaining 규정 준수.를 전문으로 하는 규정 준수 expert입니다

## Context
The user needs 에 ensure their application meets regulatory requirements 및 industry standards. Focus 에 practical 구현 of 규정 준수 controls, 자동화된 모니터링, 및 audit trail generation.
...
### 1. 규정 준수 Framework Analysis

Identify applicable regulations 및 standards:
...
**Privacy by Design**
```python
# Implement privacy 에 의해 설계 principles
class PrivacyByDesign:
    def implement_data_minimization(self):
        """
        Collect only necessary data
        """
        # 이전 (collecting too much)
        bad_user_model = {
            'email': str,
            'password': str,
            'full_name': str,
            'date_of_birth': date,
            'ssn': str,  # Unnecessary
            'address': str,  # Unnecessary 위한 기본 service
            'phone': str,  # Unnecessary
            'gender': str,  # Unnecessary
            'income': int  # Unnecessary
        }
        
        # 이후 (data minimization)
        good_user_model = {
            'email': str,  # Required 위한 authentication
    ...
    def implement_pseudonymization(self):
        """
        Replace identifying fields 와 함께 pseudonyms
        """
...
class SOC2SecurityControls:
    def implement_access_controls(self):
        """
        SOC2 CC6.1 - Logical 및 physical access controls
        """
        controls = {
            'authentication': '''
...
class MFAEnforcement:
    def enforce_mfa(self, user, resource_sensitivity):
        ...
        elif resource_sensitivity == 'medium' 및 user.is_admin:
            return self.require_mfa(user)
...
class RBACAuthorization:
    ...
    def check_permission(self, user, resource, action):
        ...
        if action 에서 user_permissions:
            return True
            
        # Check ownership-based permissions
        if f"{action}:own" 에서 user_permissions:
...
''',
            'encryption': '''
# Encryption 에서 rest 및 에서 transit
...
class ComplianceAuditLogger:
    ...
    def log_event(self, event_type, details):
        ...
        # Store 에서 immutable log
        self._store_audit_log(log_entry)
        
        # 실시간 alerting 위한 중요한 events
    ...
    def _calculate_checksum(self, entry):
        ...
        """
        # Include previous entry hash 위한 blockchain-같은 integrity
...
def generate_compliance_report(self, regulation, period):
    """
    Generate 규정 준수 report 위한 auditors
    """
    report = {
...
class HIPAACompliance:
    def protect_phi(self):
        """
        Implement HIPAA safeguards 위한 Protected Health Information
        """
...
class PHIAccessControl:
    ...
    def grant_phi_access(self, user, patient_id, purpose):
        ...
        # Grant limited access based 에 role 및 purpose
...
class PHIEncryption:
    def encrypt_phi_at_rest(self, phi_data):
        """
        HIPAA-compliant encryption 위한 PHI
        """
        ...
        # Encrypt PHI fields
        encrypted_phi = {}
        위한 field, value 에서 phi_data.items():
    ...
    def secure_phi_transmission(self):
        """
        안전한 PHI 동안 transmission
        """
        return {
            'protocols': ['TLS 1.2+'],
            'vpn_required': True,
            'email_encryption': 'S/MIME 또는 PGP required',
            'fax_alternative': '안전한 messaging portal'
        }
'''
        }
...
class HIPAATraining:
    def track_training_compliance(self, employee):
        """
        Ensure workforce HIPAA training 규정 준수
        """
        required_modules = [
            'HIPAA Privacy Rule',
            'HIPAA 보안 Rule', 
        ...
        위한 module 에서 required_modules:
            completion = self._check_module_completion(employee.id, module)
            if completion 및 completion['date'] > datetime.now() - timedelta(days=365):
...
# Network segmentation 위한 PCI 규정 준수
    ...
    def configure_network_zones(self):
        ...
        zones = {
            'cde': {  # Cardholder Data Environment
                'description': 'Systems that process, store, 또는 transmit CHD',
                'controls': [
                    'Firewall required',
                    'IDS/IPS 모니터링',
                    'No direct internet access',
                    'Quarterly vulnerability scans',
                    'Annual penetration 테스트'
                ]
            },
            'dmz': {
    ...
    def quarterly_scan_requirements(self):
        ...
        scan_config = {
            ...
            'external_scans': {
                ...
                'passing_criteria': 'Clean scan 와 함께 no failures'
            },
    ...
    def generate_realtime_dashboard(self):
        ...
        dashboard['regulations']['HIPAA'] = {
            ...
            'issues': [
                {
                    'severity': 'high',
                    'issue': '3 employees overdue 위한 training',
                    'due_date': '2025-07-25'
                }
            ]
        }
...
```yaml
# .github/workflows/규정 준수-check.yml
name: 규정 준수 Checks
...
에:
  ...
  pull_request:
  schedule:
    - cron: '0 0 * * *'  # Daily 규정 준수 check

jobs:
  규정 준수-scan:
    runs-에: ubuntu-latest
    
    steps:
    - uses: actions/checkout@v3
    
    - name: GDPR 규정 준수 Check
      run: |
        python scripts/규정 준수/gdpr_checker.py
        
    - name: 보안 Headers Check
      run: |
        python scripts/규정 준수/security_headers.py
    ...
    - name: PII Detection Scan
      run: |
        # Scan 위한 hardcoded PII
        python scripts/규정 준수/pii_scanner.py
        
    - name: Encryption Verification
      run: |
        # Verify all sensitive data is encrypted
        python scripts/규정 준수/encryption_checker.py
        
    - name: Generate 규정 준수 Report
      if: always()
      run: |
        python scripts/규정 준수/generate_report.py > 규정 준수-report.json
        
    - name: Upload 규정 준수 Report
      uses: actions/upload-artifact@v3
      와 함께:
        name: 규정 준수-report
        path: 규정 준수-report.json
```
...
## 3. Legal Basis 위한 Processing
{generate_legal_basis_section(data_practices['purposes'])}

## 4. Your Rights
Under GDPR, you have the following rights:
- Right 에 access your personal data
- Right 에 rectification 
- Right 에 erasure ('right 에 be forgotten')
- Right 에 restrict processing
- Right 에 data portability
- Right 에 object
- Rights related 에 자동화된 decision making
...
## 7. Contact Us
에 exercise your rights, contact: {company_info['privacy_email']}
"""
