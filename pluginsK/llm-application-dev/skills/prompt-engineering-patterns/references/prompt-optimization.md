...
1. **Establish Baseline**: 항상 측정 초기 성능
2. **변경 One Thing**: Isolate 변수 위한 명확한 attribution
3. **Test 철저히**: Use diverse, representative test cases
4. **Track 메트릭**: Log 모든 experiments 및 results
5. **Validate Significance**: Use statistical 테스트합니다 위한 A/B comparisons
6. **Document 변경합니다**: Keep 상세한 notes 에 무엇 및 왜
7. **버전 Everything**: Enable 롤백 에 이전 버전
8. **모니터 Production**: 지속적으로 evaluate 배포된 prompts
